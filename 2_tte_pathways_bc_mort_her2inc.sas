...
Purpose of Program: Run parametric g-formula for breast cancer mortality
                    Sensitivity analysis additionally adjusting for HER2 
                    status and income
****************************************************************************;
...
data bytimes_cens;
    set bytimes_cens;
    if income = 5 then delete;
    if her2_stat = 8 then delete;
...
fixedcov = age_dx_cat_2 age_dx_cat_3 age_dx_cat_4 age_dx_cat_5 age_dx_cat_6
    ...
    savelib = results,
    survdata = results.simsurv_bc_mort_her2inc,
    covmeandata = results.covmean_bc_mort_her2inc,
	resultsdata = results.results_bc_mort_her2inc,
...
data results;
    merge results.results_bc_mort_her2inc n_events(keep = COUNT);
...
ods tagsets.excelxp
  file = "results/2_tte_pathways_bc_mort_her2inc.xls"
  style=minimal
