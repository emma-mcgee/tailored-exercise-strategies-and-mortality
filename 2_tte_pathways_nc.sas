...
Purpose of Program: Run parametric g-formula for negative outcome control
****************************************************************************;
...
fixedcov = age_dx_cat_2 age_dx_cat_3 age_dx_cat_4 age_dx_cat_5 age_dx_cat_6
    ...
    savelib = results,
    survdata = results.simsurv_nc,
    covmeandata = results.covmean_nc,
	resultsdata = results.results_nc,
...
proc freq data =  bytimes_cens;
    tables censor / out = n_events;
    where censor = 1;
run;
* Create formatted results;
data results;
    merge results.results_nc n_events(keep = COUNT);
...
ods tagsets.excelxp
  file = "results/2_tte_pathways_nc.xls"
  style=minimal
