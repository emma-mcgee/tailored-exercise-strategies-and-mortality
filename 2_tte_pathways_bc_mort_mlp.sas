...
Purpose of Program: Run parametric g-formula for breast cancer mortality
                    sensitivity analysis with a minimum latency period (MLP)
                    of two years
****************************************************************************;
...
data bytimes_cens;
    set bytimes_cens;
    death_oth_1 = 0;
    if death_1 = 1 and death_bc_1 = 0 then death_oth_1 = 1;
run;
...
fixedcov = age_dx_cat_2 age_dx_cat_3 age_dx_cat_4 age_dx_cat_5 age_dx_cat_6
    ...
    savelib = results,
    survdata = results.simsurv_bc_mort_mlp,
    covmeandata = results.covmean_bc_mort_mlp,
	resultsdata = results.results_bc_mort_mlp,
...
data results;
    merge results.results_bc_mort_mlp n_events(keep = COUNT);
...
ods tagsets.excelxp
  file = "results/2_tte_pathways_bc_mort_mlp.xls"
  style=minimal
